...
@interface TYBasePageBarLayout ()

@property (nonatomic, weak) TYBasePageBar *pagerTabBar;
...
@implementation TYBasePageBarLayout

- (instancetype)initWithPagerTabBar:(TYBasePageBar *)pagerTabBar {
    if (self = [super init]) {
        _pagerTabBar = pagerTabBar;
        [self configurePropertys];
        self.barStyle = TYPageBarStyleProgressElasticView;
    }
    return self;
}
...
- (void)setProgressHeight:(CGFloat)progressHeight {
    ...
    frame.origin.y = _barStyle == TYPageBarStyleCoverView ? (height - _progressHeight)/2:(height - _progressHeight - _progressVerEdging);
    frame.size.height = progressHeight;
    _pagerTabBar.progressView.frame = frame;
}

- (UIEdgeInsets)sectionInset {
    if (!UIEdgeInsetsEqualToEdgeInsets(_sectionInset, UIEdgeInsetsZero) || _barStyle != TYPageBarStyleCoverView) {
        return _sectionInset;
    }
    if (_barStyle == TYPageBarStyleCoverView && _adjustContentCellsCenter) {
        return _sectionInset;
    }
...
- (void)setBarStyle:(TYPageBarStyle)barStyle
{
    if (barStyle == _barStyle) {
        return;
    }
    if (_barStyle == TYPageBarStyleCoverView) {
        self.progressBorderWidth = 0;
        self.progressBorderColor = nil;
    }
    _barStyle = barStyle;
    switch (barStyle) {
        case TYPageBarStyleProgressView:
            self.progressWidth = 0;
            self.progressHorEdging = 6;
            self.progressVerEdging = 0;
            self.progressHeight = kUnderLineViewHeight;
            break;
        case TYPageBarStyleProgressBounceView:
        case TYPageBarStyleProgressElasticView:
            self.progressWidth = 30;
            self.progressVerEdging = 0;
            self.progressHorEdging = 0;
            self.progressHeight = kUnderLineViewHeight;
            break;
        case TYPageBarStyleCoverView:
            self.progressWidth = 0;
    ...
    _pagerTabBar.progressView.hidden = barStyle == TYPageBarStyleNoneView;
    if (barStyle == TYPageBarStyleCoverView) {
        _progressRadius = 0;
...
- (void)transitionFromCell:(UICollectionViewCell<TYTabPagerBarCellProtocol> *)fromCell toCell:(UICollectionViewCell<TYTabPagerBarCellProtocol> *)toCell progress:(CGFloat)progress {
    ...
    if (self.textAnimateEnable) {
        ...
        toCell.titleLabel.font = self.selectedTextFont;
    }
...
- (void)setUnderLineFrameWithIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    CGFloat progressY = _barStyle == TYPageBarStyleCoverView ? (cellFrame.size.height - _progressHeight)/2:(cellFrame.size.height - _progressHeight - _progressVerEdging);
...
- (void)setUnderLineFrameWithfromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex progress:(CGFloat)progress
{
    ...
    CGFloat progressY = _barStyle == TYPageBarStyleCoverView ? (toCellFrame.size.height - _progressHeight)/2:(toCellFrame.size.height - _progressHeight - _progressVerEdging);
    CGFloat progressX = 0, width = 0;
    
    if (_barStyle == TYPageBarStyleProgressBounceView) {
    ...
    }else if (_barStyle == TYPageBarStyleProgressElasticView) {
        if (fromCellFrame.origin.x < toCellFrame.origin.x) {
            if (progress <= 0.5) {
